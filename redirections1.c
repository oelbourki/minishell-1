...
/*   Updated: 2020/02/04 22:57:23 by oel-bour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	help_do_somein_1(t_command **head)
{
...
int do_somein_1(t_command **head)
{
	...
	if (g_counter != NULL && g_counter->what == REDIN)
	{
		while (g_counter != NULL && g_counter->what == REDIN)
		{
			...
			if (help_do_somein_1(&g_counter) == -1)
				return (-1);
...
int	help_do_someout_1(t_command **head)
{
	...
	!= DOUBLEREDOUT && g_counter->next->str != NULL)
	{
		...
		g_counter->next->what = STRING;
		if (g_counter->next->next != NULL && (g_counter->next->next->what == REDOUT
		|| g_counter->next->next->what == DOUBLEREDOUT))
			close(g_out_fd);
	}
...
int do_someout_1(t_command **head)
{
	...
	g_counter->what == DOUBLEREDOUT))
	{
		...
		g_counter->what == DOUBLEREDOUT))
		{
			...
			if (help_do_someout_1(&g_counter) == -1)
				return (-1);
			g_counter = g_counter->next->next;
		}
		g_multi_redout = 1;
		if (g_counter != NULL && g_counter->what == REDIN && do_somein_1(&g_counter))
			return (1);
		return (1);
	}
	else 
